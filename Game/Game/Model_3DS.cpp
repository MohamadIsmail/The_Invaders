...
float Model_3DS::GetHeight(float x, float z)
{
	int X = Objects[0].Vertexes[11*3];
	int Z = Objects[0].Vertexes[11*3+2];
	int Y = Objects[0].Vertexes[11*3+1];
	/*for(int i = 0 ; i < numObjects ; i++)
	{
		for (int j = 0; j < Objects[i].numVerts * 3; j+=3)
		{
			int X = Objects[i].Vertexes[j];
			int Z = Objects[i].Vertexes[j+2];
			int Y = Objects[i].Vertexes[j+1];
			if(Objects[i].Vertexes[j] == x && Objects[i].Vertexes[j+2]== z)
				return Objects[i].Vertexes[j+1];
		}
	}*/
	return -1;
}
...
void Model_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
	...
	Materials[matindex].tex = LoadTexture(fullname);
